...
- **sub2api**：从 **GitHub Releases** 下载 [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) 的预编译二进制到 `~/opt/sub2api/bin`；默认监听 **`127.0.0.1:8802`**，保留上游 `deploy/` 文档到 `~/opt/sub2api/deploy`。安装时会优先按 release 资产清单解析平台包，并在可用时校验 `checksums.txt`；支持 `nlt-sub2api install -v v0.1.144` 指定版本、`nlt-sub2api rollback -v v0.1.143` 回滚，以及 `nlt-sub2api list-versions` 查看最近版本。运行时默认导出 `DATA_DIR=~/opt/sub2api/data`，并自动加载 `~/opt/sub2api/data/sub2api.env`（若存在）中的 `DATABASE_*` / `REDIS_*` / `JWT_SECRET` 等环境变量；示例配置会落到 `~/opt/sub2api/data/config.example.yaml`。**`run`** 为前台启动（不写 PID；后台已在跑时拒绝）。
