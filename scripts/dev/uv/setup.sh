...
[[ -f "${_UV_ROOT_DIR}/../../lib/fundeploy-install.sh" ]] || die "找不到 lib/fundeploy-install.sh"
# shellcheck source=../../lib/fundeploy-install.sh
source "${_UV_ROOT_DIR}/../../lib/fundeploy-install.sh"
...
do_install_pkg() {
  ...
  _fundeploy_say_step "使用 ${mgr} 安装 uv 包"
  _fundeploy_pm_install "${mgr}" uv || die "${mgr} 安装 uv 失败；可改用 INSTALL_METHOD=source"
  _fundeploy_say_ok "已通过 ${mgr} 安装 uv。"
}
...
do_install_source() {
  ...
  _fundeploy_say_step "执行 Astral 官方 uv 安装脚本: ${UV_INSTALL_URL}"
  # 安装器识别 UV_INSTALL_DIR、INSTALLER_NO_MODIFY_PATH 等；见上游 install.sh 注释
  curl -LsSf "${UV_INSTALL_URL}" | sh
  _fundeploy_say_ok "uv source 安装完成。"
  echo "请配置 PATH：" >&2
...
do_update_source() {
  ...
  if command -v uv >/dev/null 2>&1; then
    _fundeploy_say_step "执行 uv self update"
    uv self update
  else
    _fundeploy_say_warn "未在 PATH 中找到 uv，改为执行 source install。"
    do_install_source
  fi
}
...
do_uninstall_pkg() {
  ...
  _fundeploy_say_step "使用 ${mgr} 卸载 uv 包"
  _fundeploy_pm_uninstall "${mgr}" uv || _fundeploy_say_warn "${mgr} 卸载 uv 失败（可能本就未通过 ${mgr} 安装）。"
}

do_uninstall_source() {
  export PATH="${UV_INSTALL_DIR}:${PATH}"
  command -v uv >/dev/null 2>&1 || { _fundeploy_say_warn "未找到 uv，跳过 uv self uninstall。"; }
  if command -v uv >/dev/null 2>&1; then
    _fundeploy_say_step "执行 uv self uninstall（非交互确认）"
  if printf 'y\n' | uv self uninstall; then
      _fundeploy_say_ok "已执行 uv self uninstall。"
  else
  ...
  if [[ -d "${UV_INSTALL_DIR}" ]]; then
    ...
    _fundeploy_say_ok "已删除 source 安装目录: ${UV_INSTALL_DIR}"
  fi
}

do_install() {
  local method
  method="$(_fundeploy_resolve_method uv)"
  _fundeploy_say_title "安装 uv（方式: ${method}）"
  case "${method}" in
    pkg) do_install_pkg ;;
...
do_update() {
  local method
  method="$(_fundeploy_resolve_method uv)"
  _fundeploy_say_title "升级 uv（方式: ${method}）"
  case "${method}" in
    pkg) do_install_pkg ;;
...
do_uninstall() {
  ...
  _fundeploy_say_title "卸载 uv（方式: ${method}）"
  case "${method}" in
