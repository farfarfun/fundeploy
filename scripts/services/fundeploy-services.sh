...
cmd_status() {
  ...
  # funfluid-api / funfluid-web 各自管理自己的 PID 文件，这里只用端口探测判断是否在跑。
  ...
  PATH="${HOME}/opt/gum/bin:${PATH}"
  {
    ...
    _status_csv_line \
      "funfluid-api" \
      "$(service_state_from_pid_and_port "" "$ffl_be_listener_pid")" \
      "$(service_pid_display "" "$ffl_be_listener_pid")" \
      "${FUNFLUID_WEB_BACKEND_HOST}:${FUNFLUID_WEB_BACKEND_PORT}" \
      "$(http_probe "http://${FUNFLUID_WEB_BACKEND_HOST}:${FUNFLUID_WEB_BACKEND_PORT}/api/docs/swagger/")"
    _status_csv_line \
      "funfluid-web" \
      "$(service_state_from_pid_and_port "" "$ffl_fe_listener_pid")" \
